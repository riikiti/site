...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name_err) && empty($address_err) && empty($salary_err)) {
        ...
        $sql = "INSERT INTO `news` (`id_news`,`title`,`time`, `content`) VALUES (NULL ,?, ?, ?)";
            ...
            if ($result = mysqli_query($link, $sql2)) {
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_array($result)) {
                        $mail = $row['email'];
                        $subject = "НОВОСТЬ: $param_name";
                        $to = $mail;
                        $from = 'arkadiy@krutoi.ru';
                        $user = $row['login'];
                        $message = ("Дорогой $user, на нашем сайте вышла новость! http://site/news.php СКОРЕЕ ПРОЧИТАЙ!!!");
